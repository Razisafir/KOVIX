...
/**
 * Tests for AgentErrorRecovery — error classification and recovery.
 * Source: src/vs/platform/construct/common/recovery/agentErrorRecovery.ts
 * Implementation: src/vs/workbench/contrib/construct/browser/services/recovery/agentErrorRecovery.ts
 *
 * Tests error classification, context_window_exceeded truncation,
 * rate_limit_hit backoff, model_unavailable fallback,
 * and tool_execution_timeout retry.
 */
...
type RecoveryStrategy = 'retry' | 'skip' | 'edit' | 'abort';

interface IStepError {
	stepIndex: number;
	toolName: string;
	toolInput: unknown;
...
function handleContextWindowExceeded(
	messages: Array<{ role: string; content: string }>,
	maxTokens: number
): { truncatedMessages: Array<{ role: string; content: string }>; wasTruncated: boolean } {
...
): { delayMs: number; shouldRetry: boolean; maxRetries: number } {
	const maxRetries = 3;
...
function handleModelUnavailable(
	currentProvider: string,
	availableProviders: IProviderOption[]
): IProviderOption | null {
...
function handleToolTimeout(
	retryCount: number,
	previousTimeoutMs: number,
	maxRetries: number = 3
): { shouldRetry: boolean; newTimeoutMs: number; retryAttempt: number } {
...
function buildErrorContext(error: IStepError, previousAttempts?: string[]): string {
	const lines: string[] = [];
...
suite('AgentErrorRecovery', () => {
	...
	suite('Error classification — errors are classified correctly', () => {
		...
		test('stderr is included in classification', () => {
			assert.strictEqual(
				classifyError('Command failed', 1, 'permission denied: /etc/hosts'),
				'file_permission'
			);
		});
	});
