...
function isSudoAlternative(command: string): boolean {
        const baseCommand = command.trim().split(/\s+/)[0].split('/').pop() ?? '';
        return SUDO_ALTERNATIVES.includes(baseCommand);
}

function assertCwdJail(filePath: string, workspaceRoot?: string): void {
...
const TERMINAL_RATE_LIMIT: IRateLimitConfig = {
        maxCommands: 10,
        windowMs: 30_000,
};
...
suite('TerminalSecurity', () => {
        ...
        suite('CWD jail — workspace boundary enforcement', () => {
                test('path traversal with .. is rejected', () => {
                        assert.throws(
                                () => assertCwdJail('../../../etc/passwd', '/home/user/workspace'),
                ...
                test('path traversal with embedded .. is rejected', () => {
                        assert.throws(
                                () => assertCwdJail('src/../../../etc/passwd', '/home/user/workspace'),
                                /Path traversal not allowed/
                        );
                });
                ...
                test('absolute path outside workspace is rejected', () => {
                        assert.throws(
                                () => assertCwdJail('/etc/passwd', '/home/user/workspace'),
                                /Command targets path outside workspace/
                        );
                });
                ...
                test('absolute path without workspace context is rejected', () => {
                        assert.throws(
                                () => assertCwdJail('/usr/local/bin/something'),
