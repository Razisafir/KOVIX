...
class OllamaTestClient {
	...
	async chat(messages: Array<{ role: string; content: string }>, options?: { num_predict?: number }): Promise<{
		content: string;
		done: boolean;
	}> {
...
suite('Agent Loop E2E Tests', function () {
	...
	test('Agent can connect to Ollama and get a response', async function () {
		...
		assert.ok(
			result.content.includes('4'),
			`Response should contain "4", got: ${result.content.substring(0, 200)}`
		);
	});
	...
	test('Agent loop completes a simple code generation task', async function () {
		...
		assert.ok(
			lower.includes('function') || lower.includes('=>') || lower.includes('const'),
			`Response should contain function/arrow syntax, got: ${result.content.substring(0, 200)}`
		);
	});
	...
	test('Agent loop handles tool definitions in system prompt', async function () {
		...
		assert.ok(
			mentioned || lower.includes('tool') || lower.includes('read') || lower.includes('write'),
			`Response should mention at least one tool or tool-related term, got: ${result.content.substring(0, 300)}`
		);
	});
	...
	test('Agent loop preserves conversation context across turns', async function () {
		...
		assert.ok(
			...
			`Response should mention "React", got: ${r2.content.substring(0, 200)}`
		);
	});
