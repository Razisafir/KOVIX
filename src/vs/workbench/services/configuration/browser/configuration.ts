...
import { TASKS_CONFIGURATION_KEY, FOLDER_SETTINGS_NAME, LAUNCH_CONFIGURATION_KEY, IConfigurationCache, ConfigurationKey, REMOTE_MACHINE_SCOPES, FOLDER_SCOPES, WORKSPACE_SCOPES, APPLY_ALL_PROFILES_SETTING, LEGACY_FOLDER_CONFIG_FOLDER_NAME } from '../common/configuration.js';
...
export class FolderConfiguration extends Disposable {
        ...
        private async resolveConfigFolderAndInitialize(fileService: IFileService, uriIdentityService: IUriIdentityService, logService: ILogService, configFolderRelativePath: string): Promise<void> {
                ...
                try {
                        const constructExists = await fileService.exists(constructFolder);
                        if (!constructExists && configFolderRelativePath !== LEGACY_FOLDER_CONFIG_FOLDER_NAME) {
                                const legacyFolder = uriIdentityService.extUri.joinPath(this.workspaceFolder.uri, LEGACY_FOLDER_CONFIG_FOLDER_NAME);
                                const legacyExists = await fileService.exists(legacyFolder);
                                if (legacyExists) {
                                        this.configurationFolder = legacyFolder;
                                }
                        }
                } catch {
