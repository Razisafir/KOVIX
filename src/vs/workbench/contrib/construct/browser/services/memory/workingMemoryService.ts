...
export class WorkingMemoryService extends Disposable implements IWorkingMemoryService {
        ...
        getContextWindowSize(projectId: string): number {
                ...
                if (!ctx) { return 0; }
                return ctx.conversationHistory.reduce((sum: number, msg: string) => sum + this.estimateTokens(msg), 0);
        }
        ...
        private loadPersistedContexts(): void {
                try {
                        ...
                        const workingKeys = keys.filter((k: string) => k.startsWith(WORKING_MEMORY_PREFIX));
